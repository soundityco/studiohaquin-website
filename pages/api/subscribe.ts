...
const subscribeHandler = async (
  ...
  try {
    ...
    if (response.status == 200) {
      return res.status(201).json({ message: "Thank you! You have successfully subscribed!" });
    }
  } catch (error) {
    ...
    return res.status(500).json({
      error:
        "Oops! There was an error subscribing you to the newsletter. Please email us at contact@blurblur.show and we'll add you.",
    });
  }
};
